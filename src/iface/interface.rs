...
struct Processor<'b, 'c, 'e, 'x> {
    config: &'x Config<'c, 'e>,
    state: &'x mut State<'b, 'e>
}

macro_rules! processor {
    ($iface:expr) => (Processor{config: &$iface.config, state: &mut $iface.state})
}
...
impl<'b, 'c, 'e, DeviceT> InterfaceBuilder<'b, 'c, 'e, DeviceT>
        ...
        where T: Into<ManagedSlice<'c, IpCidr>>
    {
        ...
        Config::check_ip_addrs(&ip_addrs);
...
impl<'b, 'c, 'e, DeviceT> Interface<'b, 'c, 'e, DeviceT>
    ...
    pub fn ethernet_addr(&self) -> EthernetAddress {
        self.config.ethernet_addr.unwrap()
    }
    ...
    pub fn set_ethernet_addr(&mut self, addr: EthernetAddress) {
        ...
        Config::check_ethernet_addr(&addr);
        self.config.ethernet_addr = Some(addr);
    }
        ...
        match addr.into() {
            ...
            IpAddress::Ipv4(addr) => {
                ...
                if is_not_new {
                    ...
                    processor!(self).dispatch_ip(medium, tx_token, _timestamp, pkt)?;
                    Ok(true)
                } else {
                    Ok(false)
                }
    ...
    pub fn update_ip_addrs<F: FnOnce(&mut ManagedSlice<'c, IpCidr>)>(&mut self, f: F) {
        f(&mut self.config.ip_addrs);
        Config::check_ip_addrs(&self.config.ip_addrs)
    }

    /// Check whether the interface has the given IP address assigned.
    pub fn has_ip_addr<T: Into<IpAddress>>(&self, addr: T) -> bool {
        self.config.has_ip_addr(addr)
    }
    ...
    fn has_neighbor<'a>(&self, addr: &'a IpAddress, timestamp: Instant) -> bool {
        match self.config.route(addr, timestamp) {
            Ok(_routed_addr) => {
                match self.device.medium() {
                    #[cfg(feature = "ethernet")]
                    Medium::Ethernet => self.state.neighbor_cache.as_ref().unwrap()
    ...
    pub fn poll_at(&self, sockets: &SocketSet, timestamp: Instant) -> Option<Instant> {
        sockets.iter().filter_map(|socket| {
            ...
            match socket.meta().poll_at(socket_poll_at, |ip_addr|
                self.has_neighbor(&ip_addr, timestamp)) {
                    PollAt::Ingress => None,
        ...
        loop {
            ...
            rx_token.consume(timestamp, |frame| {
                match medium {
                    #[cfg(feature = "ethernet")]
                    Medium::Ethernet => {
                        p.process_ethernet(sockets, timestamp, &frame).map_err(|err| {
                    ...
                    Medium::Ip => {
                        p.process_ip(sockets, timestamp, &frame).map_err(|err| {
                            net_debug!("cannot process ingress packet: {}", err);
                            //net_debug!("packet dump follows:\n{}",
                            ...
                            match response {
                                Some(packet) => {
                                    processed_any = true;
                                    p.dispatch_ip(medium, tx_token, timestamp, packet).map_err(|err| {
                                        net_debug!("cannot dispatch response packet: {}", err);
                                        err
                                    })
                                }
        ...
        for mut socket in sockets.iter_mut() {
            ...
            macro_rules! respond {
                ($response:expr) => ({
                    ...
                    let tx_token = self.device.transmit().ok_or(Error::Exhausted)?;
                    device_result = processor!(self).dispatch_ip(medium, tx_token, timestamp, response);
                    device_result
                })
            }
        ...
        match self.state.igmp_report_state {
            IgmpReportState::ToSpecificQuery { version, timeout, group }
                ...
                if let Some(pkt) = self.igmp_report_packet(version, group) {
                    ...
                    let medium = self.device.medium();
                    let tx_token = self.device.transmit().ok_or(Error::Exhausted)?;
                    processor!(self).dispatch_ip(medium, tx_token, timestamp, pkt)?;
                }
...
impl<'b, 'c, 'e, 'x> Processor<'b, 'c, 'e, 'x> {
                       ...
                       Result<Option<EthernetPacket<'frame>>>
    {
        ...
        if !eth_frame.dst_addr().is_broadcast() &&
           ...
           eth_frame.dst_addr() != self.config.ethernet_addr.unwrap()
        {
            return Ok(None)
        }
        ...
        match eth_frame.ethertype() {
            ...
            EthernetProtocol::Ipv4 => {
                ...
                if eth_frame.src_addr().is_unicast() && ipv4_packet.src_addr().is_unicast() {
                    ...
                    if self.config.in_same_network(&ip_addr) {
                        self.state.neighbor_cache.as_mut().unwrap().fill(ip_addr, eth_frame.src_addr(), timestamp);
                    }
                }
            ...
            EthernetProtocol::Ipv6 => {
                ...
                if eth_frame.src_addr().is_unicast() && ipv6_packet.src_addr().is_unicast()  {
                    ...
                    if self.config.in_same_network(&ip_addr) &&
                            self.state.neighbor_cache.as_mut().unwrap().lookup_pure(&ip_addr, timestamp).is_none() {
                        self.state.neighbor_cache.as_mut().unwrap().fill(ip_addr, eth_frame.src_addr(), timestamp);
                    }
                }
        ...
        match arp_repr {
            ...
            ArpRepr::EthernetIpv4 {
                ...
                if source_protocol_addr.is_unicast() && source_hardware_addr.is_unicast() {
                    self.state.neighbor_cache.as_mut().unwrap().fill(source_protocol_addr.into(),
                ...
                if operation == ArpOperation::Request && self.config.has_ip_addr(target_protocol_addr) {
                    Ok(Some(EthernetPacket::Arp(ArpRepr::EthernetIpv4 {
                        operation: ArpOperation::Reply,
                        source_hardware_addr: self.config.ethernet_addr.unwrap(),
    ...
    fn process_ndisc<'frame>(&mut self, timestamp: Instant, ip_repr: Ipv6Repr,
        ...
        match repr {
                ...
                match lladdr {
                    Some(lladdr) if lladdr.is_unicast() && target_addr.is_unicast() => {
                        if flags.contains(NdiscNeighborFlags::OVERRIDE) {
                            self.state.neighbor_cache.as_mut().unwrap().fill(ip_addr, lladdr, timestamp)
                        } else {
                            if self.state.neighbor_cache.as_mut().unwrap().lookup_pure(&ip_addr, timestamp).is_none() {
                                    self.state.neighbor_cache.as_mut().unwrap().fill(ip_addr, lladdr, timestamp)
                            }
                        }
                    },
            ...
            NdiscRepr::NeighborSolicit { target_addr, lladdr, .. } => {
                match lladdr {
                    Some(lladdr) if lladdr.is_unicast() && target_addr.is_unicast() => {
                        self.state.neighbor_cache.as_mut().unwrap().fill(ip_repr.src_addr.into(), lladdr, timestamp)
                    },
                    _ => (),
                }
                if self.config.has_solicited_node(ip_repr.dst_addr) && self.config.has_ip_addr(target_addr) {
                    let advert = Icmpv6Repr::Ndisc(NdiscRepr::NeighborAdvert {
                        flags: NdiscNeighborFlags::SOLICITED,
                        target_addr: target_addr,
                        lladdr: Some(self.config.ethernet_addr.unwrap())
                    });
        ...
        where Tx: TxToken, F: FnOnce(EthernetFrame<&mut [u8]>)
    {
        ...
        tx_token.consume(timestamp, tx_len, |tx_buffer| {
            ...
            frame.set_src_addr(self.config.ethernet_addr.unwrap());
        ...
        where Tx: TxToken
    {
        ...
        let dst_addr = self.config.route(dst_addr, timestamp)?;

        match self.state.neighbor_cache.as_mut().unwrap().lookup(&dst_addr, timestamp) {
    ...
    fn dispatch_ip<Tx: TxToken>(&mut self, medium: Medium, tx_token: Tx, timestamp: Instant,
        ...
        let ip_repr = packet.ip_repr().lower(&self.config.ip_addrs)?;
...
mod test {
    ...
    fn test_no_icmp_no_unicast_ipv4() {
        ...
        // broadcast address
        assert_eq!(processor!(iface).process_ipv4(&mut socket_set, Instant::from_millis(0), &frame),
                   Ok(None));
    }
    ...
    fn test_no_icmp_no_unicast_ipv6() {
        ...
        // ICMP error response when the destination address is a
        // broadcast address
        assert_eq!(processor!(iface).process_ipv6(&mut socket_set, Instant::from_millis(0), &frame),
                   Ok(None));
    }
    ...
    fn test_icmp_error_port_unreachable() {
        ...
        // Ensure that the unknown protocol triggers an error response.
        // And we correctly handle no payload.
        assert_eq!(processor!(iface).process_udp(&mut socket_set, ip_repr, false, data),
                   Ok(Some(expected_repr)));
        ...
        assert_eq!(processor!(iface).process_udp(&mut socket_set, ip_repr,
    ...
    fn test_handle_udp_broadcast() {
        ...
        assert_eq!(processor!(iface).process_udp(&mut socket_set, ip_repr, false, packet.into_inner()),
                   Ok(None));
    ...
    fn test_icmp_reply_size() {
        ...
        #[cfg(all(feature = "proto-ipv4", not(feature = "proto-ipv6")))]
        assert_eq!(processor!(iface).process_udp(&mut socket_set, ip_repr.into(), false, payload),
    ...
    fn test_handle_valid_arp_request() {
        ...
        assert_eq!(processor!(iface).process_ethernet(&mut socket_set, Instant::from_millis(0), frame.into_inner()),
                   Ok(Some(EthernetPacket::Arp(ArpRepr::EthernetIpv4 {
                       operation: ArpOperation::Reply,
    ...
    fn test_icmpv4_socket() {
        ...
        assert_eq!(processor!(iface).process_icmpv4(&mut socket_set, ip_repr, icmp_data),
    ...
    fn test_solicited_node_addrs() {
        ...
        assert!(iface.config.has_solicited_node(Ipv6Address::new(0xff02, 0, 0, 0, 0, 1, 0xff00, 0x0002)));
        assert!(iface.config.has_solicited_node(Ipv6Address::new(0xff02, 0, 0, 0, 0, 1, 0xff00, 0xffff)));
        assert!(!iface.config.has_solicited_node(Ipv6Address::new(0xff02, 0, 0, 0, 0, 1, 0xff00, 0x0003)));
    }
    ...
    fn test_raw_socket_truncated_packet() {
        ...
        let frame = processor!(iface).process_ipv4(&mut socket_set, Instant::from_millis(0), &frame);
