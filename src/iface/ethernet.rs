        ...
        match addr.into() {
            ...
            IpAddress::Ipv4(addr) => {
                ...
                if is_not_new {
                    ...
                    processor!(self).dispatch(tx_token, _timestamp, EthernetPacket::Ip(pkt))?;
                    Ok(true)
                } else {
                    Ok(false)
                }
        ...
        for mut socket in sockets.iter_mut() {
            ...
            macro_rules! respond {
                ($response:expr) => ({
                    ...
                    let tx_token = self.device.transmit().ok_or(Error::Exhausted)?;
                    device_result = processor!(self).dispatch(tx_token, timestamp, response);
                    device_result
                })
            }
        ...
        match self.state.igmp_report_state {
            IgmpReportState::ToSpecificQuery { version, timeout, group }
                ...
                if let Some(pkt) = self.igmp_report_packet(version, group) {
                    // Send initial membership report
                    let tx_token = self.device.transmit().ok_or(Error::Exhausted)?;
                    processor!(self).dispatch(tx_token, timestamp, EthernetPacket::Ip(pkt))?;
                }
...
mod test {
    ...
    use super::{EthernetPacket, IpPacket, Processor};
