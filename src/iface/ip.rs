...
impl<'a> Packet<'a> {
    pub(crate) fn dst_addr(&self) -> IpAddress {
...
/// Provides a function to dispatch IP packets to lower layers.
pub trait Dispatcher {
    fn dispatch(&mut self, packet: Packet, timestamp: Instant) -> Result<()>;
}

pub struct Processor<'x, 'a, 'b, 'c> {
    pub config: &'x Config<'a, 'b>,
    pub state: &'x mut State<'c>,
}

impl<'x, 'a, 'b, 'c> Processor<'x, 'a, 'b, 'c> {
    ...
    pub fn egress(&mut self, sockets: &mut SocketSet, d: &mut impl Dispatcher, timestamp: Instant) -> Result<bool> {
        let emitted_any = self.socket_egress(sockets, d, timestamp)?;

        #[cfg(feature = "proto-igmp")]
        self.igmp_egress(d, timestamp)?;

        Ok(emitted_any)
    }

    fn socket_egress(&mut self, sockets: &mut SocketSet, d: &mut impl Dispatcher, timestamp: Instant) -> Result<bool> {
        let _caps = self.state.device_capabilities.clone();

        let mut emitted_any = false;
        for mut socket in sockets.iter_mut() {
            // FIXME(dirbaio) use real has_neighbor
            if !socket.meta_mut().egress_permitted(|_ip_addr| true) {
                continue
            }
            ...
            macro_rules! respond {
                ($response:expr) => ({
                    ...
                    neighbor_addr = Some(response.dst_addr());
                    dispatch_result = d.dispatch(response, timestamp);
                    dispatch_result
                })
            }
            ...
            match (dispatch_result, socket_result) {
    ...
    #[cfg(feature = "proto-igmp")]
    fn igmp_egress(&mut self, d: &mut impl Dispatcher, timestamp: Instant) -> Result<bool> {
    ...
    #[cfg(feature = "proto-ipv6")]
    pub fn process_ipv6<'frame> (&mut self, sockets: &mut SocketSet, timestamp: Instant, frame: &'frame [u8]) -> Result<Option<Packet<'frame>>> {
    ...
    #[cfg(feature = "proto-ipv4")]
    pub fn process_ipv4<'frame>(&mut self, sockets: &mut SocketSet, timestamp: Instant, frame: &'frame [u8]) -> Result<Option<Packet<'frame>>> {
